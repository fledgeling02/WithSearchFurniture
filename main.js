...
function displayFurnitureProducts(products) {
  ...
  if (productCount) {
    let countText = `Showing ${products.length} product${products.length !== 1 ? 's' : ''}`;
    if (searchTerm) {
      countText += ` matching "${searchInput.value.trim()}"`;
    }
      ...
      <div style="padding: 1.5rem;">
        ...
        ${product.description ? `
          <p style="color: #6b7280; font-size: 0.875rem; margin-top: 0.75rem; line-height: 1.5;">
            ${highlightSearchTerm(product.description.substring(0, 80), searchTerm)}${product.description.length > 80 ? '...' : ''}
          </p>
        ` : ''}
      </div>
    `;
...
function displayNoProducts() {
  ...
  if (container) {
    const message = hasSearch 
      ? 'No products found matching your search. Try different keywords or clear filters.'
      : 'No furniture available at the moment. Check back later.';
